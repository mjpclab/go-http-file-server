...
--restrict-access-dir <分隔符><文件系统路径>[<分隔符><允许的主机>...]
    与--global-restrict-access类似，但仅限于指定URL路径所对应的文件系统路径（及子路径）。
...
--header-dir <分隔符><文件系统路径><分隔符><名称><分隔符><值> ...
    与--header类似，但指定的是URL路径所对应的文件系统路径。
...
--auth-dir-user <分隔符><文件系统路径>[<分隔符><允许的用户名>...] ...
    对指定的URL路径所对应的文件系统路径（及子路径）启用http基本验证。

--global-list [true|false]
    对所有URL路径开启目录列表。
    默认值为"true"，设为"false"则只列出--list/--list-dir指定的路径。
--list <URL路径> ...
--list-user <分隔符><URL路径>[<分隔符><允许的用户名>...] ...
    设置允许列出目录文件的URL路径（及子路径）。
--list-dir <文件系统路径> ...
--list-dir-user <分隔符><文件系统路径>[<分隔符><允许的用户名>...] ...
    与--list类似，但指定的是URL路径所对应的文件系统路径。
...
--upload-dir-user <分隔符><文件系统路径>[<分隔符><允许的用户名>...] ...
    与--upload类似，但指定的是URL路径所对应的文件系统路径。
...
--mkdir-dir-user <分隔符><文件系统路径>[<分隔符><允许的用户名>...] ...
    与--mkdir类似，但指定的是URL路径所对应的文件系统路径。
...
--delete-dir-user <分隔符><文件系统路径>[<分隔符><允许的用户名>...] ...
    与--delete类似，但指定的是URL路径所对应的文件系统路径。
...
--archive-dir-user <分隔符><文件系统路径>[<分隔符><允许的用户名>...] ...
    与--archive类似，但指定的是URL路径所对应的文件系统路径。
...
--cors-dir <文件系统路径> ...
    接受指定的URL路径所对应的文件系统路径（及子路径）的CORS跨域请求。
...
- `↑`, `↓`：使焦点在文件项之间移动
- `PageUp`, `PageDown`: 使焦点在每页文件项之间移动
- `Ctrl`/`Opt` + `↑` 或 `Home`：把焦点移动到第一个文件项
- `Ctrl`/`Opt` + `↓` 或 `End`：把焦点移动到最后一个文件项
...
- `uploadmaxbatchcount`(=2048)：每次上传的最大子批次文件数
- `uploadmaxbatchsize`：每次上传的最大子批次文件大小，单位字节，例如“100M”、“1G”，未设置或为0表示不限制
